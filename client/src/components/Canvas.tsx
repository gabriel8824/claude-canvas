...
import { CanvasNode, Group, TerminalData, FilesData, PreviewData, GitData, EditorData, DocsData } from '../types';
...
function GroupsLayer({ groups, nodes, highlightId, onClose, onStartGroupDrag, onUpdateGroup }: {
  groups: Group[];
  nodes: CanvasNode[];
        ...
        return (
          <div
            ...
            style={{
              ...
              borderRadius: 24,
              border: `1px solid rgba(${rgb},${hi ? 0.4 : hov ? 0.22 : 0.13})`,
              background: `rgba(${rgb},${hi ? 0.09 : 0.04})`,
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, padding: '0 0 0 14px', cursor: 'grab', overflow: 'hidden', height: '100%' }}
              >
...
function ConnectionsLayer({ nodes }: { nodes: CanvasNode[] }) {
  ...
  const allConns = [...editorConns, ...previewConns];
...
export function Canvas() {
  const { nodes, groups, updateNode, bringToFront, removeNode, toggleMinimize, addNodeToGroup,
          canvasOffset, setCanvasOffset, zoom, setZoom, removeGroup, updateGroup, setNodesPositions } = useCanvasStore();
          ...
          {nodes.map(node => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${activeId === node.id ? 'rgba(140,185,255,0.35)' : 'rgba(255,255,255,0.08)'}`,
              ...
              onMouseDown={() => { bringToFront(node.id); setActiveId(node.id); }}
            >
              <div
              ...
              >
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <div onClick={e => { e.stopPropagation(); removeNode(node.id); }} title="Close" style={trafficLight('rgba(248,113,113,0.9)')} />
